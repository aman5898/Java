...
public class Prac {
		...
		for (int i = end; i >= 0; i--) {
			for (int j = end; j >= 0; j--) {
				if (i == end && j == end) {
					...
					path[i][j] = costs[i][j] + Math.min(path[i + 1][j], path[i][j + 1]);
			}
		}
	...
	public static void minCostPathPrint(int row, int col, int[][] costs, String asf) {
		...
		for (int i = 0; i < strg.length; i++) {
			for (int j = 0; j < strg[0].length; j++) {
				if (i == 0) {
					...
					if (j - wts[i - 1] >= 0) {
						strg[i][j] = Math.max(strg[i - 1][j], strg[i - 1][j - wts[i - 1]] + prices[i - 1]);
					} else {
						strg[i][j] = strg[i - 1][j];
					}
	...
	private static void printKnapsackPath(int row, int col, int[][] strg, int[] wts, int[] prices, String psf) {
		if (row == 0 || col == 0) {
			System.out.println(psf);
			return;
		}
		...
		if (max1 > max2) {
			...
			printKnapsackPath(row - 1, col - wts[row - 1], strg, wts, prices, psf + " " + wts[row - 1]);
		} else {
			printKnapsackPath(row - 1, col, strg, wts, prices, psf);
			printKnapsackPath(row - 1, col - wts[row - 1], strg, wts, prices, psf + " " + wts[row - 1]);
		}
	...
	public static int findLongestFromACell(int row, int col, int mat[][], int dp[][]) {
		...
		if (row < mat.length - 1) {
			if (mat[row][col] + 1 == mat[row + 1][col]) {
				return dp[row][col] = 1 + findLongestFromACell(row + 1, col, mat, dp);
			}
		}
		...
		if (col < mat.length - 1) {
			if (mat[row][col] + 1 == mat[row][col + 1]) {
				return dp[row][col] = 1 + findLongestFromACell(row, col + 1, mat, dp);
			}
		}
	...
	public static int LongPalinSubseqMemo(int sIdx, int lIdx, String str, int[][] qb) {
		if (sIdx == lIdx) {
			return 1;
		}
		if (sIdx > lIdx) {
		...
		if (ch1 == ch2) {
			return qb[sIdx][lIdx] = LongPalinSubseqMemo(sIdx + 1, lIdx - 1, str, qb) + 2;
		} else {
			return qb[sIdx][lIdx] = Math.max(LongPalinSubseqMemo(sIdx, lIdx - 1, str, qb),
	...
	public static int eggDrop(int e, int f, int[][] qb) {
		...
		int min = Integer.MAX_VALUE;
		for (int i = 1; i <= f; i++) {
			min = Math.min(min, Math.max(eggDrop(e - 1, i - 1, qb), eggDrop(e, f - i, qb)));
		}
	...
	public static void maxSumNoAdjacentElement(int vidx, int ans, boolean prev, int[] arr) {
		if (vidx == arr.length) {
