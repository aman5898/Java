...
//		Node middle;
	...
	public void diagonalTraverse() {
		...
		diagonalTraverse(root, map, 0);
		ArrayList<Integer> list = new ArrayList<>(map.keySet());
		for (int i = 0; i < list.size(); i++) {
			LinkedList<Integer> llist = map.get(list.get(i));
	...
	private void diagonalTraverse(Node node, HashMap<Integer, LinkedList<Integer>> map, int level) {
		if (node == null) {
			return;
		}
		...
		diagonalTraverse(node.left, map, level + 1);
		diagonalTraverse(node.right, map, level);
	}

	public void specificOrdreLevelTraversal() {
		HashMap<Integer, LinkedList<Integer>> map = new HashMap<>();
		specificOrdreLevelTraversal(root, map, 0);
		System.out.print(root.data + " ");
		ArrayList<Integer> list = new ArrayList<>(map.keySet());
		for (int i = 1; i < list.size(); i++) {
			LinkedList<Integer> llist = map.get(list.get(i));
			for (int j = 0, k = llist.size() - 1; j < llist.size() / 2; j++, k--) {
				System.out.print(llist.get(j) + " " + llist.get(k) + " ");
			}
		}
	}

	private void specificOrdreLevelTraversal(Node node, HashMap<Integer, LinkedList<Integer>> map, int level) {
		if (node == null) {
			return;
		}
		...
		specificOrdreLevelTraversal(node.left, map, level + 1);
		specificOrdreLevelTraversal(node.right, map, level + 1);
	}

	public void specificOrdreLevelTraversal2() {
		...
		// use it as queue enqueue as addLast dequeue as remove first
		while (!q.isEmpty()) {
			Node one = q.removeFirst();
			Node two = q.removeFirst();
			System.out.print(one.data + " " + two.data + " ");
			if (one.left != null) {
				q.addLast(one.left);
				q.addLast(two.right);
				q.addLast(one.right);
	...
	public static int vidx = -1;

	public void specialTreeFromGivenPreorderTraversal(int[] pre, char[] preLn) {
		int vidx = -1;
		root = specialTreeFromGivenPreorderTraversal(pre, preLn, 5);
	}

	private Node specialTreeFromGivenPreorderTraversal(int[] pre, char[] preLn, int a) {
		vidx++;
		if (preLn[vidx] == 'L') {
			Node temp = new Node();
			temp.data = pre[vidx];
			...
			Node node = new Node();
			node.data = pre[vidx];
			node.left = specialTreeFromGivenPreorderTraversal(pre, preLn, a);
			node.right = specialTreeFromGivenPreorderTraversal(pre, preLn, a);
			return node;
		}
	}
	...
	public void binaryTreeToDoublyLinkedList() {
	...
	private int leftSumTree(Node node) {
		if (node == null) {
			return 0;
		}
...
//			lastVisited = node;
//		} else {
//			node.middle = lastVisited;
//			lastVisited = node;
//		}
//		inorderSuccessor(node.left);
//	}

	public void connectAtSameLevel(Node node) {
		if (node == null) {
			return;
		}
		if (node.left != null) {
